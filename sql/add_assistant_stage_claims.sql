-- ════════════════════════════════════════════════════════════════════════════
-- เพิ่มแผนก "ผู้ช่วยช่าง" ในงานเคลมด้วย (ต่อจาก sql/add_assistant_stage.sql ของหมวดออเดอร์)
-- สายงานเคลมใหม่: รอของคืน → ตัดผ้าแล้ว → เย็บแล้ว → **ตรวจสอบแล้ว** → รีดแล้ว → แพ็คแล้ว → ส่งแล้ว
--   (ชุดเดียวกับ WORKFLOW ใน components/ClaimsWorkspace.tsx + components/mobile/MobileClaims.tsx)
--
-- ไฟล์นี้ = ของเดิมใน sql/claim_scan.sql ทุกอย่าง เปลี่ยนแค่ flow + stage map
-- ‼️ รันใน Supabase → SQL Editor (รันซ้ำได้ ปลอดภัย) — แทนที่ claim_scan_advance / claim_scan_join เดิม
-- ════════════════════════════════════════════════════════════════════════════

-- ===== 1) เดินสถานะงานเคลม =====
create or replace function public.claim_scan_advance(
  p_claim_id  uuid,
  p_stage_key text,   -- cut | sew | assist | iron | pack | shipped
  p_tech_code text,
  p_tech_name text
) returns jsonb
language plpgsql
...
-- ════════════════════════════════════════════════════════════════════════════
-- เสร็จ — ผู้ช่วยช่างสแกน QR ใบเคลมได้แล้ว (สถานะเคลมขึ้น "ตรวจสอบแล้ว")
-- ════════════════════════════════════════════════════════════════════════════
