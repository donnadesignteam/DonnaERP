...
//
// กติกาที่ user เคาะไว้ (2026-07-31, ปรับ 2026-08-03):
// - โบนัสของออเดอร์อยู่กับ "แอดมินหลักคนล่าสุดที่แก้รายการสินค้า (items)"
//   แก้อย่างอื่น (ที่อยู่/หมายเหตุ/ราคา ฯลฯ) ชื่อแอดมินไม่ขยับ
// - ‼️ คนที่ไม่ใช่แอดมินหลักมาแก้รายการสินค้า = **ล้างเจ้าของโบนัสทิ้ง ไม่มีใครได้โบนัสใบนั้น**
...
export function stampUpdate(patch: Row): Row {
  ...
  if (touchesContent(patch)) {
    ...
    // กติกาปรับ 3 ส.ค. 69: เจ้าของโบนัสย้าย/ถูกล้างเฉพาะตอนแก้ "รายการสินค้า" เท่านั้น
    // แก้ที่อยู่ หมายเหตุ ราคา ฯลฯ ชื่อแอดมินเดิมคงอยู่ (เดิมแตะเนื้อออเดอร์อะไรก็ล้าง โดนใบที่แค่แก้ที่อยู่)
    if ('items' in patch) {
      if (s && isBonusAdmin(s.code)) {
        out.admin_code = s.code
        out.admin_name = s.nickname
      } else {
        // ‼️ คนนอกแอดมินหลักแก้รายการสินค้า = ล้างโบนัสทิ้ง ไม่ใช่คงเจ้าของเดิม
        out.admin_code = null
        out.admin_name = null
      }
    }
