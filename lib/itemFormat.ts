...
const FIXED_TYPES: [RegExp, string][] = [
  ...
  // สินค้าตกแต่ง (ตามหน้าร้าน Shopee) — ชื่อหน้าสินค้ายาว/ติดสีมาด้วย → เหลือชื่อสินค้าชื่อเดียว สีไปช่องสี
  // เรียงเฉพาะเจาะจงก่อน: "เบาะนั่งมินิมอล หมอนปุยเมฆ" คือสินค้าตัวเดียว (หมอนปุยเมฆ = ชื่อรุ่น ไม่ใช่หมอนแถม)
  [/หนม\s*ปัง/, 'เบาะนั่งน้องหนมปัง'],
  [/ฟองน้ำ|muji|มูจิ|โซฟา/i, 'เบาะนั่งฟองน้ำ'],
  [/ผูกปม|หมอนอิง/, 'หมอนผูกปม'],
  [/ปุยเมฆ|^เบาะ/, 'เบาะนั่งมินิมอล'],
  [/วอล\s*เป|วอลล์|wall\s*paper/i, 'วอลเปเปอร์'],
]
...
export function normalizeItemType(type: string): string {
  ...
  const isMain = /^(ราง|ม่าน|ผ้า|มู่ลี่|มุ้ง|กล่อง|เบาะ|หมอน|วอล)/.test(t)
...
export const FIELD_CHOICES: Record<string, string[]> = {
  type: [
    ...
    'กล่องบังราง',
    'เบาะนั่งมินิมอล', 'เบาะนั่งน้องหนมปัง', 'เบาะนั่งฟองน้ำ', 'หมอนผูกปม', 'วอลเปเปอร์',
