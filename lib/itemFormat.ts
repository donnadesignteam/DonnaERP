...
export function railKind(type: string): 'รางจีบ' | 'รางลอนเทป' | 'รางตาไก่' | null {
  ...
  // เครื่องคำนวณมีแค่ 3 ชนิด — ราง "ลอน" ทุกแบบ (ลอนเทป/ลอนตะขอ/ลอนโซ่ไข่ปลา) ใช้ชุดรางลอนเทป
  // (ตรงกับ railMap.js ฝั่ง donna-rail) ถ้าไม่แมป จะเด้งเตือน "อ่านชนิดรางไม่ออก" แล้วคิดเป็นรางจีบผิด
  if (t.includes('ลอน') || t.includes('เทป')) return 'รางลอนเทป'
  if (t.includes('จีบ')) return 'รางจีบ'
  return null   // อ่านไม่ออก (รางไฟฟ้า/รางมอเตอร์) — ให้ผู้เรียกตัดสินใจเอง ห้ามเดาเป็นรางจีบเงียบๆ
}
