...
export default function OrderWorkspace({ scope = 'orders' }: { scope?: 'orders' | 'claims' }) {
  ...
  const displayedOut = (() => {
    ...
    if (createdSort) {
      rs = [...rs].sort((a, b) => cmpCreated(a, b, createdSort))
    } else if (outUpdatedSort) {
      rs = [...rs].sort((a, b) => {
        const da = a.updated_at ? new Date(a.updated_at).getTime() : 0
        const db = b.updated_at ? new Date(b.updated_at).getTime() : 0
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border)', background: 'transparent' }}>
                ...
                {showCol('created') && (
                <th style={{ textAlign: 'left', padding: '10px 14px', fontWeight: 500, whiteSpace: 'nowrap' }}>
                  <button onClick={e => openOutFilter(e, 'out-created')}
                    style={{ border: 'none', background: 'transparent', fontSize: 12, fontWeight: 500, color: createdSort ? 'var(--blue)' : 'var(--ink-3)', cursor: 'pointer', padding: 0, display: 'flex', alignItems: 'center', gap: 3 }}>
                    วันที่สร้าง <span style={{ fontSize: 9, opacity: 0.6 }}>▼</span>
                  </button>
                </th>
                )}
                {showCol('outsource') && (
            ...
            <tbody>
              {pageSlice(displayedOut).map(r => {
                ...
                const isCancelled = r.order_status === 'ยกเลิก'
                return (
                  <tr key={r.id} className={r.pinned ? 'is-pinned' : undefined} style={{ borderBottom: '1px solid var(--border)', background: selectedIds.has(r.id) ? 'var(--blue-bg)' : r.pinned ? '#F1E2D2' : 'transparent' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border)', background: 'transparent' }}>
                ...
                {showCol('pay_date') && (
                <th style={{ textAlign: 'left', padding: '10px 14px', fontWeight: 500, whiteSpace: 'nowrap', position: 'relative' }}>
                  <button onClick={() => setOpenFilter(openFilter === 'pay-date' ? null : 'pay-date')}
                    style={{ border: 'none', background: 'transparent', fontSize: 12, fontWeight: 500, color: createdSort ? 'var(--blue)' : 'var(--ink-3)', cursor: 'pointer', padding: 0, display: 'flex', alignItems: 'center', gap: 3 }}>
        ...
        return (
          <AnchoredMenu rect={actionRect} className="ow-drop">
            ...
            {/* ยกเลิกออเดอร์ = เปลี่ยนสถานะเป็น "ยกเลิก" (ไปอยู่แท็บยกเลิก ไม่ลบทิ้ง) — กดซ้ำเอากลับมาได้
                งานเคลมไม่มีสถานะยกเลิก ให้ไปจัดการที่หน้าเคลม */}
            {!isClaimEntry(r) && (() => {
              ...
              return (
                <button onClick={async () => {
                  ...
                  {cancelled ? 'เอากลับจากยกเลิก' : 'ยกเลิกออเดอร์'}
                </button>
              )
            })()}
