...
export default function MobileInstallations() {
  ...
  const [zone, setZone] = useState<string>('')       // '' = ทุกโซน
    ...
    <div>
      <div style={{ position: 'sticky', top: 0, zIndex: 50, background: 'var(--bg)', paddingTop: 'env(safe-area-inset-top)', borderBottom: '1px solid var(--border)' }}>
        <div style={{ padding: '12px 14px 8px', display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 10 }}>
          <h1 style={{ fontSize: 20, fontWeight: 700, color: 'var(--ink)' }}>ปฏิทินงานติดตั้ง</h1>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 10px 8px', gap: 8 }}>
          ...
          <button onClick={goToday} style={{ flex: 1, border: 'none', background: 'transparent', cursor: 'pointer', padding: '4px 0' }}>
            <span style={{ fontSize: 15.5, fontWeight: 700, color: 'var(--ink)' }}>{TH_MONTHS[month]} {year + 543}</span>
            <span style={{ fontSize: 11.5, color: 'var(--ink-4)', marginLeft: 7 }}>{monthCount} งาน</span>
          </button>
          ...
          {['', ...ZONES].map(z => {
            ...
            return (
              <button key={z || 'all'} onClick={() => setZone(z)}
                style={{ flexShrink: 0, padding: '5px 12px', borderRadius: 999, cursor: 'pointer', fontSize: 12.5, whiteSpace: 'nowrap',
          ...
          {cells.map((day, i) => {
            ...
            return (
              <button key={i} onClick={() => setDaySheet(ymd)}
                style={{
                  minHeight: 62, display: 'flex', flexDirection: 'column', gap: 2,
                ...
                <span style={{ fontSize: 12, fontWeight: isToday ? 800 : 500, color: isToday ? 'var(--blue)' : holiday ? '#b45309' : 'var(--ink-2)' }}>{day}</span>
                {jobs.slice(0, 2).map(j => {
                  const c = INSTALL_STATUS_COLOR[j.installation_status ?? ''] ?? 'var(--ink-3)'
                  return (
                    <span key={j.id} style={{ fontSize: 8.5, fontWeight: 700, lineHeight: 1.25, borderRadius: 3, padding: '1px 3px',
                ...
                {jobs.length > 2 && <span style={{ fontSize: 8.5, color: 'var(--ink-4)' }}>+{jobs.length - 2}</span>}
              </button>
            )
          })}
        </div>
      ...
      {daySheet && (
        <div onClick={() => setDaySheet(null)}
          ...
          <div onClick={e => e.stopPropagation()}
            ...
            {HOLIDAYS[daySheet] && <div style={{ fontSize: 12.5, color: '#b45309', fontWeight: 600, marginBottom: 8 }}>🏖️ วันหยุดร้าน · {HOLIDAYS[daySheet]}</div>}
                    ...
                    <div key={j.id} style={{ border: '1px solid var(--border)', borderLeft: `3px solid ${c}`, borderRadius: 10, padding: '10px 12px', background: 'var(--bg)' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 8 }}>
                        <span style={{ fontSize: 14.5, fontWeight: 700, color: 'var(--ink)' }}>
                          {timeOf(j)} {j.customer_real_name || j.customer_id || 'ไม่ระบุชื่อ'}
                        </span>
                      ...
                      {j.work_details && <div style={{ fontSize: 12.5, color: 'var(--ink-2)', marginTop: 6, lineHeight: 1.45 }}>{j.work_details}</div>}
                      {itemLines.length > 0 && (
                      ...
                      {j.notes && <div style={{ fontSize: 12, color: 'var(--ink-3)', marginTop: 5, lineHeight: 1.45 }}>หมายเหตุ: {j.notes}</div>}
...
const linkBtn: React.CSSProperties = {
  fontSize: 12.5, fontWeight: 600, color: 'var(--blue)', textDecoration: 'none',
  border: '1px solid var(--border)', borderRadius: 8, padding: '5px 10px', background: 'var(--surface)',
}
